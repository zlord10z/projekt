...
sql_statements = open("sqlstatements.txt","r")
sql_statements = sql_statements.read()
sql_statements = sql_statements.split("$")
for i in range(len(sql_statements)):
    sql_statements[i] = sql_statements[i].replace('\n', '')
...
class BazaDanych:
    ...
    def selecta2(self, x,dm,dp,gm,gp):
        cursor2 = self.connection.cursor()
        ...
        string2 = str(se1 + "WHERE public.user.user_id = '" + str(x) + "' AND now() - interval '" + str(dm) + " days' > time_stamp AND now() - interval'" + str(dp) + " days' < time_stamp")
        string2 = str(string2 + " and now() - interval '"+str(gm)+" hours' > time_stamp AND now() - interval'"+str(gp)+" hours' < time_stamp " )
    ...
    def selecta5(self,s_inf,s_id):
        ...
        s4 = ("where health_condition "+s_inf+"\' and time_stamp > now() - interval \'20 week\' and public.user_location.user_id = "+s_id)
    ...
    def selecta6(self,user_id,infection):
        cursor = self.connection.cursor()

        s1 = ("select public.health_condition.health_condition,public.user_location.time_stamp , public.user.user_id,  public.location.latitude, public.location.longtitude from public.user ")
        s2 = ("join public.user_location on public.user.user_id = public.user_location.user_id join public.location on public.user_location.location_id = public.location.location_id ")
        s3 = ("join public.health_condition on public.user.user_id = public.health_condition.health_condition_id ")
        #s4 = ("where health_condition "+str(infection)+"\' and time_stamp > now() - interval \'1 hour\' and public.user_location.user_id = "+str(user_id))
        s4 = ("where health_condition " + str(infection) + "\'  and public.user_location.user_id = " + str(user_id))
        string = s1 + s2 + s3 + s4
        string = str(string)

        cursor.execute(string)
        self.select6 = cursor.fetchall()



    def selecta7(self,user_id):
        cursor = self.connection.cursor()
        s1 = str(sql_statements[0])
        s2 = ("Where public.user.user_id =" + str(user_id) + "ORDER BY user_location.time_stamp DESC LIMIT 1;")
        string = s1 + s2
        cursor.execute(string)
        self.select7 = cursor.fetchall()


    def selecta8(self, user_id, day_minus,day_plus,hour_minus,hour_plus):
        cursor = self.connection.cursor()
        s1 = str(sql_statements[1])
        s2 = ("public.user.user_id =" + str(user_id) + "and time_stamp between '"+str(day_minus)+"' and '"+str(day_plus)+"' and date_part ('hour',time_stamp) BETWEEN "+str(hour_minus)+" AND "+str(hour_plus))
        string = s1 + s2
        cursor.execute(string)
        self.select8 = cursor.fetchall()

    def selecta9(self):
        cursor = self.connection.cursor()
        string = sql_statements[2]
        cursor.execute(string)
        self.select9 = cursor.fetchall()
...
class Api:
    def api_get_waypoints(self, lat_1, long_1, lat_2, long_2):
        to_api = (str(lat_1) + "," + str(long_1) + "&end=" + str(lat_2) + "," + str(long_2))
        headers = {
            'Accept': 'application/json, application/geo+json, application/gpx+xml, img/png; charset=utf-8',
        }
        call = requests.get(
            "https://api.openrouteservice.org/v2/directions/foot-walking?api_key="+str(api_key[0])+'&start=' + str(to_api), headers=headers)
        ...
        for feature in gj['features']:
            ...
            self.l = feature['geometry']['coordinates']



    def api_holidays(self):
        ...
        for i in range(2):
            ...
            if len(response.content) == 2:
                self.holidays.append("false")
            else:
                self.holidays.append("true")
